...
DF <- data.table::as.data.table(readRDS("./data/20210603.FrequencyMutations.byTissue.RDS"))
setorder(DF,-counts, Gene)
...
writeTissueCSV <- function(tissue){
  ...
  setorderv(subDF, c(colnames(subDF)[3],"Gene"), order = c(-1,1)) 
